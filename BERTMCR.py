...
class BERTMCR:
    def __init__(self, naq_threshold: float = 2.0):
        ...
        self.__model = TFAutoModelForQuestionAnswering.from_pretrained(self.__bert_model)
    ...
    def extract_answers(self, context: str, questions: List[str]) -> Dict[str, List]:
        ...
        for question in tqdm(questions, desc="Extracting answers from context..."):
            ...
            inputs = self.__tokenizer(question, context, add_special_tokens=True, return_tensors="tf", max_length=512,
                                      truncation=True)
...
st.write("A model to get answers from questions given a context. Powered by Google's BERT.")

with st.form(key='input_form'):
    context = st.text_area('Context', height=100, help="Enter context here...")
